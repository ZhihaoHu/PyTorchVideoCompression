...
eccv, = plt.plot(bpp, psnr, "b-*", linewidth=LineWidth, label='CW_ECCV')
...
iccv, = plt.plot(bpp, psnr, "c-*", linewidth=LineWidth, label='AD_ICCV')
...
if not os.path.exists(prefix):
    os.makedirs(prefix)
plt.legend(handles=[h264, h265, DVC, eccv, iccv, MLVC], loc=4)
plt.grid()
plt.xlabel('Bpp')
plt.ylabel('PSNR')
plt.title('UVG dataset')
# plt.savefig(savepathpsnr + '.eps', format='eps', dpi=300, bbox_inches='tight')
plt.savefig(savepathpsnr + '.png', dpi=60)
plt.clf()
...
eccv, = plt.plot(bpp, msssim, "b-*", linewidth=LineWidth, label='CW_ECCV')

bpp = [0.052080695, 0.060782631, 0.073709049, 0.094923348, 0.132789571, 0.211082522]
msssim = [0.953062573, 0.958916648, 0.964496266, 0.969689354, 0.974711348, 0.979329442]
iccv, = plt.plot(bpp, msssim, "c-*", linewidth=LineWidth, label='AD_ICCV')
...
ha_iccv, = plt.plot(bpp, msssim, "g-o", linewidth=LineWidth, label='AH_ICCV')
...
plt.legend(handles=[h264, h265, DVC, eccv, ha_iccv, iccv, MLVC], loc=4)
plt.grid()
plt.xlabel('Bpp')
plt.ylabel('MS-SSIM')
plt.title('UVG dataset')
# plt.savefig(savepathmsssim + '.eps', format='eps', dpi=300, bbox_inches='tight')
plt.savefig(savepathmsssim + '.png', dpi=60)
plt.clf()

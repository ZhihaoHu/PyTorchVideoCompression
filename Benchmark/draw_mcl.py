...
iccv19, = plt.plot(bpp, psnr, "c-*", linewidth=LineWidth, label='AD_ICCV')
...
if not os.path.exists(prefix):
    os.makedirs(prefix)
plt.legend(handles=[h264, h265, DVC, iccv19, MLVC], loc=4)
plt.grid()
plt.xlabel('Bpp')
plt.ylabel('PSNR')
plt.title('MCL-JCV dataset')
plt.savefig(savepathpsnr, dpi=75)
...
psnr = [34.33114855, 35.00107532, 35.7351181, 36.56075116, 37.47248012, 38.5945216, 39.43083815]#, 40.29353013]
msssim = [0.956373977, 0.96103441, 0.966706579, 0.971418613, 0.976173806, 0.982212426, 0.984971718]#, 0.987377043]
iccv19, = plt.plot(bpp, msssim, "c-*", linewidth=LineWidth, label='AD_ICCV')
...
plt.legend(handles=[h264, h265, DVC, iccv19], loc=4)
plt.grid()
plt.xlabel('Bpp')
plt.ylabel('MS-SSIM')
plt.title('MCL-JCV dataset')
plt.savefig(savepathmsssim, dpi=75)
